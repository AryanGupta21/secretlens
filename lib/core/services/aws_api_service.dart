...
// ─────────────────────────────────────────────────────────────────────────────
// Service — wraps https://code-gaurd.onrender.com
// ─────────────────────────────────────────────────────────────────────────────
...
class AwsApiService {
  ...
  AwsApiService({http.Client? client}) : _client = client ?? http.Client();
  ...
  Future<List<StoredSecretItem>> listSecrets({String? serviceFilter}) async {
    final uri = Uri.parse('$_baseUrl/api/v1/secrets').replace(
      queryParameters:
          serviceFilter != null ? {'service_filter': serviceFilter} : null,
    );
  ...
  Future<StoreSecretResult> storeAndGenerateCode({
    ...
    final uri = Uri.parse('$_baseUrl/api/v1/secrets/store-and-generate-code');
